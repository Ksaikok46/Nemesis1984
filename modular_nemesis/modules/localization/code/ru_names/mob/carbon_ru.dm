/mob/living/carbon/alien/get_ru_names()
	return alist(
		NOMINATIVE = "ксеноморф",
		GENITIVE = "ксеноморфа",
		DATIVE = "ксеноморфу",
		ACCUSATIVE = "ксеноморфа",
		INSTRUMENTAL = "ксеноморфом",
		PREPOSITIONAL = "ксеноморфе",
	)

/mob/living/carbon/alien/adult/drone/get_ru_names()
	return alist(
		NOMINATIVE = "ксеноморф-трутень",
		GENITIVE = "ксеноморфа-трутня",
		DATIVE = "ксеноморфу-трутню",
		ACCUSATIVE = "ксеноморфа-трутня",
		INSTRUMENTAL = "ксеноморфом-трутнем",
		PREPOSITIONAL = "ксеноморфе-трутне",
	)

/mob/living/carbon/alien/adult/hunter/get_ru_names()
	return alist(
		NOMINATIVE = "ксеноморф-охотник",
		GENITIVE = "ксеноморфа-охотника",
		DATIVE = "ксеноморфу-охотнику",
		ACCUSATIVE = "ксеноморфа-охотника",
		INSTRUMENTAL = "ксеноморфом-охотником",
		PREPOSITIONAL = "ксеноморфе-охотнике",
	)

/mob/living/carbon/alien/adult/royal/praetorian/get_ru_names()
	return alist(
		NOMINATIVE = "ксеноморф-преторианец",
		GENITIVE = "ксеноморфа-преторианца",
		DATIVE = "ксеноморфу-преторианцу",
		ACCUSATIVE = "ксеноморфа-преторианца",
		INSTRUMENTAL = "ксеноморфом-преторианцем",
		PREPOSITIONAL = "ксеноморфе-преторианце",
	)

/mob/living/carbon/alien/adult/royal/queen/get_ru_names()
	return alist(
		NOMINATIVE = "королева ксеноморфов",
		GENITIVE = "королевы ксеноморфов",
		DATIVE = "королеве ксеноморфов",
		ACCUSATIVE = "королеву ксеноморфов",
		INSTRUMENTAL = "королевой ксеноморфов",
		PREPOSITIONAL = "королеве ксеноморфов",
	)

/mob/living/carbon/alien/adult/sentinel/get_ru_names()
	return alist(
		NOMINATIVE = "ксеноморф-страж",
		GENITIVE = "ксеноморфа-стража",
		DATIVE = "ксеноморфу-стражу",
		ACCUSATIVE = "ксеноморфа-стража",
		INSTRUMENTAL = "ксеноморфом-стражем",
		PREPOSITIONAL = "ксеноморфе-страже",
	)

/mob/living/carbon/alien/larva/get_ru_names()
	return alist(
		NOMINATIVE = "грудолом",
		GENITIVE = "грудолома",
		DATIVE = "грудолому",
		ACCUSATIVE = "грудолома",
		INSTRUMENTAL = "грудоломом",
		PREPOSITIONAL = "грудоломе",
	)
